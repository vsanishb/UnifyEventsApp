...
class _AddToCartFlowState extends ConsumerState<AddToCartFlow> {
  ...
  Widget _buildCountStep() {
    ...
    return Padding(
      ...
      child: Column(
        children: [
          Text("PARTICIPANT COUNT", style: GoogleFonts.plusJakartaSans(color: Colors.white, fontSize: 18, fontWeight: FontWeight.w900)),
          const SizedBox(height: 8),
          Text(
            isFixed ? "Fixed group of $lower" : "Choose between $lower and $upper",
          ...
          if (!isFixed)
            Row(
              ...
              children: [
                ...
                Padding(
                  ...
                  child: Text("$_participantCount", style: GoogleFonts.jetBrainsMono(color: Colors.white, fontSize: 40, fontWeight: FontWeight.w900)),
                ),
            ...
            Text("$_participantCount", style: GoogleFonts.jetBrainsMono(color: Colors.white, fontSize: 40, fontWeight: FontWeight.w900)),
  ...
  Widget _buildDetailsStep() {
    return Container(
      ...
      child: SingleChildScrollView(
        ...
        child: Column(
          children: [
            Text("PARTICIPANT DETAILS", style: GoogleFonts.plusJakartaSans(color: Colors.white, fontSize: 18, fontWeight: FontWeight.w900)),
            const SizedBox(height: 24),
            ...
            Row(
              children: [
                ...
                Expanded(child: ElevatedButton(onPressed: _next, child: const Text("CONTINUE"))),
              ],
            ),
          ],
        ),
  ...
  Widget _participantForm(int index) {
    return Container(
      ...
      child: Column(
        ...
        children: [
          Text("PARTICIPANT ${index + 1}", style: GoogleFonts.plusJakartaSans(color: const Color(0xFF00E5FF), fontSize: 10, fontWeight: FontWeight.w800, letterSpacing: 1)),
          const SizedBox(height: 16),
          TextFormField(
  ...
  Widget _buildSlotStep() {
    ...
    return Padding(
      ...
      child: Column(
        children: [
          Text("SELECT TIMESLOT", style: GoogleFonts.plusJakartaSans(color: Colors.white, fontSize: 18, fontWeight: FontWeight.w900)),
          const SizedBox(height: 24),
          if (slots.isEmpty)
            Text("No slots available for this event.", style: GoogleFonts.plusJakartaSans(color: Colors.white54))
          else
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.circular(12),
          border: Border.all(color: isSelected ? const Color(0xFFFF1C7C) : Colors.white.withOpacity(0.05), width: 1.5),
        ),
        child: Row(
          children: [
            Icon(Icons.access_time, color: isSelected ? const Color(0xFFFF1C7C) : Colors.white38, size: 20),
            const SizedBox(width: 16),
            Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text("${_formatTime(slot.startTime)} - ${_formatTime(slot.endTime)}", style: GoogleFonts.jetBrainsMono(color: Colors.white, fontSize: 14, fontWeight: FontWeight.bold)),
                Text("${slot.availableParticipants ?? '∞'} SLOTS LEFT", style: GoogleFonts.plusJakartaSans(color: Colors.white38, fontSize: 10, fontWeight: FontWeight.bold)),
              ],
            ),
            const Spacer(),
            if (isSelected) const Icon(Icons.check_circle, color: Color(0xFFFF1C7C)),
          ],
        ),
      ),
    );
  ...
  void _complete() async {
    try {
      await ref.read(cartActionProvider).addToCart(
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text("Added to cart successfully!"), backgroundColor: Color(0xFF39FF14)),
        );
      }
    } catch (e) {
      if (mounted) {
